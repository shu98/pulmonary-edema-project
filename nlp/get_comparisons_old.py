...
def get_search_radius(sentence, match_obj, radius):
    start = sentence[:max(match_obj.start()-1,0)]
    end = sentence[min(match_obj.end()+1,len(sentence)-1):]

    start_split = start.split(" ")
    start_split = start_split[-max(0, radius):]

    end_split = end.split(" ")
    end_split = end_split[:max(0, radius)]
